  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="glass-panel p-3 sm:p-4">
        <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-full border border-slate-200 py-2 sm:py-2.5 pl-9 sm:pl-10 pr-4 text-sm focus:border-[var(--chelsea-blue)] focus:outline-none focus:ring-2 focus:ring-[var(--chelsea-blue)] focus:ring-opacity-20"
            />
          </div>
          <button
            onClick={() => setShowFilters(!showFilters)}
            className="flex items-center justify-center gap-2 rounded-full border-2 border-[var(--chelsea-blue)] px-4 py-2 sm:px-6 sm:py-2.5 text-sm font-semibold text-[var(--chelsea-blue)] hover:bg-[var(--chelsea-blue)] hover:text-white transition-all whitespace-nowrap"
          >
            <SlidersHorizontal className="h-4 w-4 flex-shrink-0" />
            <span>Filters</span>
            {activeFiltersCount > 0 && (
              <span className="rounded-full bg-[var(--orange-cta)] px-2 py-0.5 text-xs text-white">
                {activeFiltersCount}
              </span>
            )}
          </button>
        ...
        {/* Filter Panel */}
        {showFilters && (
          <div className="mt-3 sm:mt-4 grid gap-3 sm:gap-4 border-t border-slate-200 pt-3 sm:pt-4 sm:grid-cols-2 lg:grid-cols-4">
            {/* Fuel Type */}
      ...
      <div className="grid gap-3 sm:gap-4">
        {filteredVehicles.length > 0 ? (
          filteredVehicles.map((vehicle) => (
            <article key={vehicle.id} className="glass-panel p-4 sm:p-6 text-sm text-slate-600 hover:shadow-xl transition-shadow">
              <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-2 sm:gap-3">
                <h2 className="text-xl sm:text-2xl font-semibold text-[var(--chelsea-blue)] min-w-0">
                  <Link href={`/cars/${vehicle.id}`} className="hover:text-[var(--orange-cta)] transition-colors break-words">
                    {vehicle.title}
                  </Link>
                </h2>
                <p className="text-lg sm:text-xl font-semibold text-[var(--chelsea-blue)] flex-shrink-0">£{vehicle.price.toLocaleString()}</p>
              </div>
              <p className="text-xs uppercase tracking-[0.3em] text-slate-400 mt-1">{vehicle.status}</p>
              <p className="mt-2 text-xs sm:text-sm">
                {vehicle.year} · {vehicle.fuel} · {vehicle.transmission} · {vehicle.mileage.toLocaleString()} miles
              </p>
              <p className="mt-3 text-sm leading-relaxed">{vehicle.description}</p>
              <ul className="mt-3 flex flex-wrap gap-2 text-xs">
                {vehicle.features.slice(0, 4).map((feature) => (
                  <li key={feature} className="rounded-full border border-slate-200 px-2.5 py-1 sm:px-3">
                    {feature}
                  </li>
                ))}
              </ul>
              <div className="mt-4">
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-[var(--chelsea-blue)] px-5 py-2 sm:px-6 sm:py-2.5 text-sm font-semibold text-white hover:bg-[var(--orange-cta)] transition-colors"
                >
                  View Details
                </Link>
              </div>
          ...
          <div className="glass-panel p-8 sm:p-12 text-center">
            <p className="text-base sm:text-lg font-semibold text-slate-900 mb-2">No vehicles found</p>
